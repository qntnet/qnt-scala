...
//      )
  ...
  def calcCorrelation(relativeReturns: Series[LocalDate, Double]): IndexedSeq[Map[String, Any]] = {
    ...
    val subLst = net.OBJECT_MAPPER.readValue(
      net.httpRequest(engineUrl + "?participantId=" + participantId),
      classOf[Seq[Map[String, Any]]]).map(s => s("id")
    )
    val rrBytes = netcdf.seriesToNetcdf(relativeReturns)
    val rrGz = gzip.gzipCompress(rrBytes)
    val rrB64 = Base64.getEncoder.encodeToString(rrGz)

    val r = Map[String, AnyRef]("relative_returns" -> rrB64, "submission_ids" -> subLst)

    val responseBytes = net.httpRequest(statanUrl, r)

    net.OBJECT_MAPPER.readValue(responseBytes, classOf[IndexedSeq[Map[String, Any]]])
  }
